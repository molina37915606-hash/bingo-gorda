...
(async()=>{try{
 ...
 const view=await req('/api/admin/player-view-session',{method:'POST',headers:admin,body:JSON.stringify({playerId:ai.id})});assert.equal(view.response.status,200,JSON.stringify(view.data));assert.equal(view.data.readOnly,true);assert.equal(view.data.virtual,true);assert(view.data.url.includes('adminpreview=1'));assert(!/codigo=|acceso=|simcontrol=/i.test(view.data.url));
