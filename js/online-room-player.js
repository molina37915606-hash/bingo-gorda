...
class PlayerApp {
  ...
  finishPlayerBoot() {
    clearTimeout(window.__bingoPlayerBootWatchdog);
    delete document.documentElement.dataset.playerSessionBoot;
    const mask = $('playerBootMask');
    mask?.classList.remove('show','error');
    window.__BINGO_PLAYER_BOOT_READY__ = true;
  }

  showPlayerBootFailure(error) {
    console.error('Error iniciando pantalla del jugador:', error);
    const mask = $('playerBootMask');
    const title = $('playerBootTitle');
    const text = $('playerBootText');
    if (title) title.textContent = 'No se pudo abrir la sala';
    if (text) text.textContent = 'Tu sesión sigue guardada. Tocá REINTENTAR para recuperar la partida.';
    mask?.classList.add('show','error');
    document.documentElement.dataset.playerSessionBoot = '1';
  }
  ...
  applyState(data) {
    ...
    try {
      // Preparar primero toda la interfaz. Solo después hacemos visible el juego.
      // Así una excepción nunca deja al usuario mirando una pantalla completamente vacía.
      this.render(); this.renderDemoUi(); this.renderChat(); this.renderPublicClaim(); this.renderInfoDrawer(); this.renderSystemTrust();
      $('loginView').classList.add('hidden'); $('gameView').classList.remove('hidden');
      document.body.classList.add('playerLogged');
      $('infoDrawerToggle').classList.remove('hidden');
      this.finishPlayerBoot();
      if (!this.adminPreviewMode) this.ensureMarkingModeChoice();
      this.handleOwnPrizeReadiness(); if (!this.adminPreviewMode) this.trackManualLag(); this.handleTestEvent(); this.handleSequence(previous); this.updateWakeLock();
    } catch (error) {
      this.showPlayerBootFailure(error);
      return;
    }
  ...
  renderPlaying() {
    if (this.state?.player?.excludedFromRound) {
      $('playPanel').classList.add('hidden');
      $('waitingPanel').classList.remove('hidden');
      $('waitingPanel').innerHTML = `<div class="waitingConfirmed waitingStateHero"><b>ESTA RONDA YA INICIÓ</b><div>Tu participación no estaba confirmada al comenzar${this.state?.roomSettings?.paymentMode==='paid'?' (pago pendiente)':''}. No recibiste cartones en esta ronda.</div><small style="display:block;margin-top:10px">Contactá al administrador para la próxima partida.</small></div>`;
      return;
    }
