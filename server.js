...
const COMMUNITY_EMPTY_MANUAL_IDLE_MS = Math.max(TEST_MODE ? 200 : 60_000, Number(process.env.BINGO_COMMUNITY_EMPTY_IDLE_MS || 5 * 60 * 1000));
...
const AUTO_PRIZE_LEAD_IN_MS = Math.max(TEST_MODE ? 35 : 500, Number(process.env.BINGO_AUTO_PRIZE_LEAD_IN_MS || (TEST_MODE ? 70 : 1500)));
const AUTO_PRIZE_SHOW_MS = Math.max(TEST_MODE ? 80 : 1800, Number(process.env.BINGO_AUTO_PRIZE_SHOW_MS || (TEST_MODE ? 140 : 4200)));
const AUTO_PRIZE_BETWEEN_MS = Math.max(TEST_MODE ? 25 : 300, Number(process.env.BINGO_AUTO_PRIZE_BETWEEN_MS || (TEST_MODE ? 55 : 850)));
...
const SOLO_TTL_MS = 4 * 60 * 60 * 1000;
const SOLO_IDLE_TTL_MS = 45 * 60 * 1000;
const SOLO_AI_NAMES = Object.freeze(['Mateo', 'Zoe', 'Owen']);
const WAITING_MINIGAME_TYPES = Object.freeze(['red_black', 'higher_lower', 'gorda_21', 'ghost_ball', 'secret_number', 'intruder_ball']);
...
function analyzeCard(card, drawnValues, playerMarks = []) {
  ...
  return {
    ...
    hasQuadrupleLine: Number(card.mode) === 75 && lineCount >= 4,
    hasQuintupleLine: Number(card.mode) === 75 && lineCount >= 5,
...
function loadState(stateFile = OWNER_STATE_FILE) {
  try {
    ...
    const merged = {
      ...
      prizeAnnouncement: parsed.prizeAnnouncement && typeof parsed.prizeAnnouncement === 'object' ? parsed.prizeAnnouncement : null,
    ...
    if (merged.championship?.enabled) {
      ...
      merged.championship.firstCornersDrawnCount = Math.max(0, Number(merged.championship.firstCornersDrawnCount) || 0) || null;
      merged.championship.firstSecondLineDrawnCount = Math.max(0, Number(merged.championship.firstSecondLineDrawnCount) || 0) || null;
      merged.championship.firstTripleLineDrawnCount = Math.max(0, Number(merged.championship.firstTripleLineDrawnCount) || 0) || null;
      merged.championship.firstQuadrupleLineDrawnCount = Math.max(0, Number(merged.championship.firstQuadrupleLineDrawnCount) || 0) || null;
      merged.championship.firstQuintupleLineDrawnCount = Math.max(0, Number(merged.championship.firstQuintupleLineDrawnCount) || 0) || null;
...
function normalizeCommunityPublicRoom(raw = {}) {
  ...
  return {
    ...
    inviteToken: /^invite_[a-f0-9]{24}$/i.test(String(raw.inviteToken || '')) ? String(raw.inviteToken) : '',
    ...
    autoSeconds: gameKind === 'championship' ? CHAMPIONSHIP_AUTO_SECONDS : gameKind === 'flash' ? FLASH_AUTO_SECONDS : Math.max(4, Math.min(20, Math.round(Number(raw.autoSeconds) || 8))),
...
function ensureWorkspace(id = 'owner', operatorId = null, label = 'Administrador principal') {
  ...
  const workspace = {
    ...
    isSolo: safe.startsWith('solo_'),
    expiresAt: safe.startsWith('demo_') ? Date.now() + DEMO_TTL_MS : (safe.startsWith('solo_') ? Date.now() + SOLO_TTL_MS : null),
...
function registrationSummaryPayload() {
  const registered = state.players.filter(player => competitiveCommunityPlayer(player));
...
function activePrizeLabelsFor(modeValue = state.game?.mode, rulesValue = state.game?.rules, linePrizeCountValue = state.roomSettings?.linePrizeCount) {
  const mode = Number(modeValue) === 75 ? 75 : 90;
  const rules = rulesValue && typeof rulesValue === 'object' ? rulesValue : {};
  const labels = [];
  if (mode === 90) {
    if (rules.ambocabeza) labels.push('AMBO CABEZA');
    if (rules.line) labels.push(Math.max(1, Math.min(2, Number(linePrizeCountValue) || 1)) === 2 ? '2 LÍNEAS' : 'LÍNEA');
    if (rules.bingo !== false) labels.push('BINGO');
  } else {
    if (rules.corners) labels.push('4 ESQUINAS');
    if (rules.line) labels.push('LÍNEA');
    if (rules.doubleLine) labels.push('DOBLE LÍNEA');
    if (rules.tripleLine) labels.push('TRIPLE LÍNEA');
    if (championshipRoomEnabled()) labels.push('CUÁDRUPLE LÍNEA', 'QUINTA LÍNEA');
    if (rules.bingo !== false) labels.push('BINGO');
  }
  return labels;
}

function automaticPrizeAnnouncementPayload() {
  const announcement = state.prizeAnnouncement;
  if (!announcement || typeof announcement !== 'object') return null;
  const items = Array.isArray(announcement.items) ? announcement.items : [];
  const currentIndex = Math.max(0, Math.min(Math.max(0, items.length - 1), Number(announcement.currentIndex) || 0));
  const cleanItem = item => item ? {
    id: String(item.id || ''),
    type: item.type,
    prizeNumber: Number(item.prizeNumber) || 1,
    prizeLabel: String(item.prizeLabel || prizeLabelFor(item.type, item.prizeNumber, state.game?.mode)),
    winners: (Array.isArray(item.winners) ? item.winners : []).map(winner => ({
      claimId: String(winner.claimId || ''), playerId: String(winner.playerId || ''), playerName: String(winner.playerName || ''),
      cardId: String(winner.cardId || ''), cardNumber: winner.cardNumber
    }))
  } : null;
  return {
    id: String(announcement.id || ''), batchId: String(announcement.batchId || ''), ball: announcement.ball ?? null,
    drawnCount: Number(announcement.drawnCount) || 0, stage: String(announcement.stage || 'lead_in'),
    currentIndex, totalItems: items.length, createdAt: announcement.createdAt || null,
    announceAt: announcement.announceAt || null, showStartedAt: announcement.showStartedAt || null, showEndsAt: announcement.showEndsAt || null,
    current: cleanItem(items[currentIndex]), items: items.map(cleanItem).filter(Boolean)
  };
}
...
function adminPayload() {
  ...
  return {
    ...
    prizeAnnouncement: automaticPrizeAnnouncementPayload(),
...
function communityCreatorPlayersPayload(viewer) {
  ...
  return (state.players || [])
    .filter(item => competitiveCommunityPlayer(item))
    .map(item => ({
...
function playerPayload(player) {
  ...
  return {
    ...
    communityRoom: state.roomSettings?.roomOrigin === 'community' ? {
      ...
      kind: soloCommunityRoomEnabled() ? 'solo' : (state.roomSettings?.communityAccessType === 'private' ? 'private' : 'public'),
      isSolo: soloCommunityRoomEnabled(),
      playMode: state.roomSettings?.communityPlayMode || 'room',
      ...
      shareUrl: soloCommunityRoomEnabled() ? '' : (state.roomSettings?.communityPublicId ? communityPublicShareUrl(state.roomSettings.communityPublicId) : `${PUBLIC_URL || `http://localhost:${PORT}`}/jugador?sala=${encodeURIComponent(state.roomCode)}&directo=1`),
      ...
      participants: soloCommunityRoomEnabled() ? state.players.filter(item => competitiveCommunityPlayer(item)).map(item => ({ name:playerDisplayName(item), virtual:Boolean(item.virtual), ready:Boolean(item.selectionConfirmed), cardCount:championshipRoomEnabled() && state.championship?.stage === 'registration' ? Math.max(1, Number(item.requestedCardCount)||Number(item.allowedCardCount)||1) : (item.cardIds || []).length })) : null,
      prizeLabels: activePrizeLabelsFor(),
...
function soloCommunityRoomEnabled() {
  return Boolean(state?.active && state.roomSettings?.roomOrigin === 'community' && state.roomSettings?.communityPlayMode === 'solo');
}

function competitiveCommunityPlayer(player) {
  return Boolean(player?.nameSet && (!player.virtual || soloCommunityRoomEnabled()));
}

function virtualSoloAutomationEnabled() {
  return Boolean(currentWorkspace().isDemo || soloCommunityRoomEnabled());
}
...
function repairChampionship75CurrentRound() {
  ...
  for (const position of champ.positions || []) {
    ...
    if (!entry.quadrupleLineBall) {
      const ball = earliestChampionshipCompletionBall(card, drawn, analysis => Number(analysis.lineCount) >= 4);
      if (ball) { entry.quadrupleLineBall = ball; changed = true; }
    }
    if (!entry.quintupleLineBall) {
      const ball = earliestChampionshipCompletionBall(card, drawn, analysis => Number(analysis.lineCount) >= 5);
      if (ball) { entry.quintupleLineBall = ball; changed = true; }
    }
  ...
  const quadrupleBalls = entries.map(entry => Number(entry.quadrupleLineBall) || 0).filter(Boolean);
  const quintupleBalls = entries.map(entry => Number(entry.quintupleLineBall) || 0).filter(Boolean);
  ...
  const firstQuadruple = Number(champ.firstQuadrupleLineDrawnCount) || (quadrupleBalls.length ? Math.min(...quadrupleBalls) : 0);
  const firstQuintuple = Number(champ.firstQuintupleLineDrawnCount) || (quintupleBalls.length ? Math.min(...quintupleBalls) : 0);
  ...
  if (firstQuadruple && !champ.firstQuadrupleLineDrawnCount) { champ.firstQuadrupleLineDrawnCount = firstQuadruple; changed = true; }
  if (firstQuintuple && !champ.firstQuintupleLineDrawnCount) { champ.firstQuintupleLineDrawnCount = firstQuintuple; changed = true; }
  for (const entry of entries) {
    ...
    const quadrupleBonus = firstQuadruple && Number(entry.quadrupleLineBall) === firstQuadruple ? CHAMPIONSHIP_SCORE.firstQuadrupleLine : 0;
    const quintupleBonus = firstQuintuple && Number(entry.quintupleLineBall) === firstQuintuple ? CHAMPIONSHIP_SCORE.firstQuintupleLine : 0;
    ...
    if (Number(entry.firstQuadrupleLineBonus || 0) !== quadrupleBonus) { entry.firstQuadrupleLineBonus = quadrupleBonus; changed = true; }
    if (Number(entry.firstQuintupleLineBonus || 0) !== quintupleBonus) { entry.firstQuintupleLineBonus = quintupleBonus; changed = true; }
    recomputeChampionshipEntryPoints(entry, 75);
  }
...
function antibingoParticipants() {
  ...
  const rows = [];
  for (const player of (state.players || []).filter(item => competitiveCommunityPlayer(item) && item.selectionConfirmed && !item.excludedFromRound)) {
    for (const cardId of player.cardIds || []) {
...
function flashLeaderboard() {
  ...
  return (state.players || []).filter(player => competitiveCommunityPlayer(player) && player.selectionConfirmed && flashPlayerCard(player)).map(player => {
    const card = flashPlayerCard(player);
...
function resetFlashSuddenDeathSeries() {
  ...
  const contenders = (state.players || []).filter(player => contenderIds.has(String(player.id)) && competitiveCommunityPlayer(player));
...
function createChampionshipState(payload = {}, mode = 90) {
  ...
  return {
    ...
    firstQuadrupleLineDrawnCount: null,
    firstQuintupleLineDrawnCount: null,
...
function championshipPointsFor(mode, type) {
  ...
  if (Number(mode) === 75 && type === 'quadrupleLine') return CHAMPIONSHIP_SCORE.quadrupleLine;
  if (Number(mode) === 75 && type === 'quintupleLine') return CHAMPIONSHIP_SCORE.quintupleLine;
...
function championshipPositionStats(position) {
  ...
  const quadrupleRounds = rounds.filter(item => Number(item.quadrupleLineBall) > 0);
  const quintupleRounds = rounds.filter(item => Number(item.quintupleLineBall) > 0);
  ...
  return {
    ...
    quadrupleLines: quadrupleRounds.length,
    quadrupleLinePoints: quadrupleRounds.reduce((sum,item)=>sum+Math.max(0,Number(item.quadrupleLinePoints)||0),0),
    firstQuadrupleLineBonuses: rounds.filter(item=>Number(item.firstQuadrupleLineBonus)>0).length,
    firstQuadrupleLineBonusPoints: rounds.reduce((sum,item)=>sum+Math.max(0,Number(item.firstQuadrupleLineBonus)||0),0),
    quintupleLines: quintupleRounds.length,
    quintupleLinePoints: quintupleRounds.reduce((sum,item)=>sum+Math.max(0,Number(item.quintupleLinePoints)||0),0),
    firstQuintupleLineBonuses: rounds.filter(item=>Number(item.firstQuintupleLineBonus)>0).length,
    firstQuintupleLineBonusPoints: rounds.reduce((sum,item)=>sum+Math.max(0,Number(item.firstQuintupleLineBonus)||0),0),
...
function championshipLeaderboard() {
  ...
  return sorted.map((position, index) => {
    ...
    return {
      ...
      quadrupleLines: stats.quadrupleLines,
      quadrupleLinePoints: stats.quadrupleLinePoints,
      firstQuadrupleLineBonuses: stats.firstQuadrupleLineBonuses,
      firstQuadrupleLineBonusPoints: stats.firstQuadrupleLineBonusPoints,
      quintupleLines: stats.quintupleLines,
      quintupleLinePoints: stats.quintupleLinePoints,
      firstQuintupleLineBonuses: stats.firstQuintupleLineBonuses,
      firstQuintupleLineBonusPoints: stats.firstQuintupleLineBonusPoints,
      lines: stats.lines,
      ...
      quadrupleLineBall: entry?.quadrupleLineBall || null,
      quintupleLineBall: entry?.quintupleLineBall || null,
      ...
      firstQuadrupleLineBonus: Math.max(0, Number(entry?.firstQuadrupleLineBonus) || 0),
      firstQuintupleLineBonus: Math.max(0, Number(entry?.firstQuintupleLineBonus) || 0),
...
function championshipRoundLeaderboard() {
  ...
  const sorted = (state.championship.positions || []).filter(position => !position.retired).map(position => {
    ...
    return {
      ...
      quintupleLineBall: entry?.quintupleLineBall || null, quintupleLinePoints:Math.max(0,Number(entry?.quintupleLinePoints)||0), firstQuintupleLineBonus:Math.max(0,Number(entry?.firstQuintupleLineBonus)||0),
      quadrupleLineBall: entry?.quadrupleLineBall || null, quadrupleLinePoints:Math.max(0,Number(entry?.quadrupleLinePoints)||0), firstQuadrupleLineBonus:Math.max(0,Number(entry?.firstQuadrupleLineBonus)||0),
...
function championshipPublicPayload(player = null) {
  ...
  const own = player ? all.filter(item => item.playerId === player.id).map(item => {
    ...
    return {
      ...
      quadrupleLinePoints: Math.max(0, Number(entry?.quadrupleLinePoints) || 0),
      quintupleLinePoints: Math.max(0, Number(entry?.quintupleLinePoints) || 0),
      ...
      quadrupleLineClaimed: Boolean(entry?.quadrupleLineClaimedAt),
      quadrupleLineClaimedAt: entry?.quadrupleLineClaimedAt || null,
      quintupleLineClaimed: Boolean(entry?.quintupleLineClaimedAt),
      quintupleLineClaimedAt: entry?.quintupleLineClaimedAt || null,
  ...
  return {
    ...
    firstQuadrupleLineDrawnCount:Number(champ.firstQuadrupleLineDrawnCount)||null,
    firstQuintupleLineDrawnCount:Number(champ.firstQuintupleLineDrawnCount)||null,
    ...
    roundSummaries:(Array.isArray(champ.roundHistory)?champ.roundHistory:[]).map(item=>({
      ...
      firstQuadrupleLineDrawnCount:Number(item.firstQuadrupleLineDrawnCount)||null, firstQuintupleLineDrawnCount:Number(item.firstQuintupleLineDrawnCount)||null,
...
function recomputeChampionshipEntryPoints(entry, mode) {
  ...
  entry.quadrupleLinePoints = is75 && entry.quadrupleLineBall ? championshipPointsFor(mode, 'quadrupleLine') : 0;
  entry.quintupleLinePoints = is75 && entry.quintupleLineBall ? championshipPointsFor(mode, 'quintupleLine') : 0;
  ...
  entry.firstQuadrupleLineBonus = is75 && entry.quadrupleLineBall ? Math.max(0, Number(entry.firstQuadrupleLineBonus) || 0) : 0;
  entry.firstQuintupleLineBonus = is75 && entry.quintupleLineBall ? Math.max(0, Number(entry.firstQuintupleLineBonus) || 0) : 0;
  entry.firstBingoBonus = entry.bingoBall ? Math.max(0, Number(entry.firstBingoBonus) || 0) : 0;
  entry.points = entry.hitPoints + entry.linePoints + entry.firstLineBonus + entry.cornersPoints + entry.firstCornersBonus + entry.secondLinePoints + entry.firstSecondLineBonus + entry.tripleLinePoints + entry.firstTripleLineBonus + entry.quadrupleLinePoints + entry.firstQuadrupleLineBonus + entry.quintupleLinePoints + entry.firstQuintupleLineBonus + entry.bingoPoints + entry.firstBingoBonus;
  return entry.points;
}
...
function ensureChampionshipPositions() {
  ...
  champ.positions = [];
  for (const player of state.players.filter(player => competitiveCommunityPlayer(player))) {
...
function prepareChampionshipRound(roundNumber) {
  ...
  champ.firstQuadrupleLineDrawnCount = null;
  champ.firstQuintupleLineDrawnCount = null;
  ...
  positions.forEach((position, index) => {
    ...
    if (!existing) {
      ...
      position.rounds.push({
        ...
        quadrupleLineBall:null, quadrupleLinePoints:0, firstQuadrupleLineBonus:0, quadrupleLineClaimedAt:null,
        quintupleLineBall:null, quintupleLinePoints:0, firstQuintupleLineBonus:0, quintupleLineClaimedAt:null,
...
function processChampionshipAfterDraw() {
  ...
  const newQuadrupleLineEntries = [];
  const newQuintupleLineEntries = [];
  ...
  for (const position of champ.positions || []) {
    ...
    if (is75 && !entry.quadrupleLineBall && analysis.lineCount >= 4) {
      entry.quadrupleLineBall = drawCount;
      newQuadrupleLineEntries.push(entry);
    }
    if (is75 && !entry.quintupleLineBall && analysis.lineCount >= 5) {
      entry.quintupleLineBall = drawCount;
      newQuintupleLineEntries.push(entry);
    }
  ...
  if (is75 && !champ.firstQuadrupleLineDrawnCount && newQuadrupleLineEntries.length > 0) {
    champ.firstQuadrupleLineDrawnCount = drawCount;
    for (const entry of newQuadrupleLineEntries) {
      entry.firstQuadrupleLineBonus = CHAMPIONSHIP_SCORE.firstQuadrupleLine;
      recomputeChampionshipEntryPoints(entry, mode);
    }
    logEvent('championship_first_quadruple_line', { round:champ.currentRound, ballOrder:drawCount, ballNumber:state.game.drawn.at(-1), bonus:CHAMPIONSHIP_SCORE.firstQuadrupleLine, simultaneous:newQuadrupleLineEntries.length });
  }
  if (is75 && !champ.firstQuintupleLineDrawnCount && newQuintupleLineEntries.length > 0) {
    champ.firstQuintupleLineDrawnCount = drawCount;
    for (const entry of newQuintupleLineEntries) {
      entry.firstQuintupleLineBonus = CHAMPIONSHIP_SCORE.firstQuintupleLine;
      recomputeChampionshipEntryPoints(entry, mode);
    }
    logEvent('championship_first_quintuple_line', { round:champ.currentRound, ballOrder:drawCount, ballNumber:state.game.drawn.at(-1), bonus:CHAMPIONSHIP_SCORE.firstQuintupleLine, simultaneous:newQuintupleLineEntries.length });
  }
...
function createChampionshipClaim(player, payload = {}) {
  ...
  const mode = Number(state.game.mode) === 75 ? 75 : 90;
  const allowedTypes = mode === 75 ? ['line','corners','secondLine','tripleLine','quadrupleLine','quintupleLine','bingo'] : ['line','secondLine','bingo'];
  const type = allowedTypes.includes(requested) ? requested : '';
  if (!type) throw new Error(mode === 75 ? 'En Campeonato 75 sólo se puede cantar Primera Línea, 4 Esquinas, Segunda Línea, Triple Línea, Cuádruple Línea, Quinta Línea o Bingo.' : 'En Campeonato sólo se puede cantar Primera Línea, Segunda Línea o Bingo.');
  ...
  const claimOne = (pos, currentEntry, claimType) => {
    const configs = {
      ...
      quadrupleLine:{ ballKey:'quadrupleLineBall', claimedKey:'quadrupleLineClaimedAt', pointsKey:'quadrupleLinePoints', bonusKey:'firstQuadrupleLineBonus', label:'CUÁDRUPLE LÍNEA' },
      quintupleLine:{ ballKey:'quintupleLineBall', claimedKey:'quintupleLineClaimedAt', pointsKey:'quintupleLinePoints', bonusKey:'firstQuintupleLineBonus', label:'QUINTA LÍNEA' },
...
function createGeneratedGame(payload = {}) {
  ...
  const flash = payload.roomOrigin === 'community' && payload.communityGameKind === 'flash';
  return {
    id: randomId('game'), number: 1, mode, rules,
    drawMode: payload.drawMode === 'manual' ? 'manual' : 'automatic',
    autoSeconds: championship ? CHAMPIONSHIP_AUTO_SECONDS : flash ? FLASH_AUTO_SECONDS : Math.max(2, Math.min(60, Number(payload.autoSeconds) || 6)),
    presenter: PRESENTER_ID,
...
function waitingGamePayload() {
  ...
  const leaderboards = {};
  for (const type of WAITING_MINIGAME_TYPES) {
    leaderboards[type] = sortBoard(game.leaderboards?.[type] || (game.type === type ? legacy : []));
  }
  const aggregateByPlayer = new Map();
  for (const type of WAITING_MINIGAME_TYPES) {
    for (const entry of leaderboards[type]) {
  ...
  return {
    type: 'all',
    activeTypes: [...WAITING_MINIGAME_TYPES],
    leaderboards,
...
function createSimpleRoom(payload = {}) {
  ...
  replaceCurrentState({
    ...
    roomSettings: {
      ...
      communityGameKind,
      communityPlayMode: payload.roomOrigin === 'community' && payload.communityPlayMode === 'solo' ? 'solo' : 'room',
    ...
    waitingGame: { type: 'all', leaderboard: [], leaderboards: Object.fromEntries(WAITING_MINIGAME_TYPES.map(type => [type, []])) },
    drawOrder: createSecureDrawOrder(game.mode), game, players: [], cardReservations: {}, claims: [], eventLog: [],
    chat: { enabled: true, locked: false, messages: [], mutedPlayerIds: [], lastSentAt: {} },
...
function openJoinPlayer(payload = {}) {
  ...
  // Un ingreso real es actividad de la sala. En salas comunitarias manuales vacías,
  // esta marca reinicia el plazo de limpieza antes de agregar al jugador.
  currentWorkspace().lastActivityAt = Date.now();
...
function submitWaitingGameScore(player, payload = {}) {
  ...
  const gameType = WAITING_MINIGAME_TYPES.includes(payload.gameType) ? payload.gameType : 'red_black';
  const score = Math.max(0, Math.min(9999, Math.floor(Number(payload.score) || 0)));
  state.waitingGame ||= { type: 'all', leaderboard: [], leaderboards: Object.fromEntries(WAITING_MINIGAME_TYPES.map(type => [type, []])) };
  state.waitingGame.type = 'all';
  state.waitingGame.leaderboards ||= Object.fromEntries(WAITING_MINIGAME_TYPES.map(type => [type, []]));
...
function rivalsNormalPayload(viewer) {
  ...
  const prizes = prizeStatusPayload();
  const rows = [];
  for (const player of (state.players || []).filter(item => competitiveCommunityPlayer(item) && item.selectionConfirmed && String(item.id) !== String(viewer?.id || ''))) {
    let best = null;
    for (const cardId of player.cardIds || []) {
      const card = state.game.cards.find(item => item.id === cardId);
      if (!card) continue;
...
function scheduleAutomaticDraw() {
  ...
  const effectiveSeconds = championshipRoomEnabled() ? CHAMPIONSHIP_AUTO_SECONDS : flashRoomEnabled() ? FLASH_AUTO_SECONDS : Math.max(minimumSeconds, Math.min(60, Number(state.game.autoSeconds) || 6));
  if (championshipRoomEnabled()) state.game.autoSeconds = CHAMPIONSHIP_AUTO_SECONDS;
  else if (flashRoomEnabled()) state.game.autoSeconds = FLASH_AUTO_SECONDS;
...
function virtualPlayersAreActive() {
  return Boolean(virtualSoloAutomationEnabled() || state.roomSettings?.adminSimulation);
}
...
function scheduleSimulatedAiChat() {
  if (!(virtualSoloAutomationEnabled() || state.roomSettings?.adminSimulation) || !state.roomSettings?.simulatedChat || state.chat?.enabled === false || state.chat?.locked) return 0;
  const workspace = currentWorkspace();
  if (virtualSoloAutomationEnabled() && !TEST_MODE) {
    const now = Date.now();
  ...
  const maxBurst = virtualSoloAutomationEnabled() ? 1 : Math.min(4, Math.max(1, Math.ceil(players.length / 20)));
  ...
  chosen.forEach((player, index) => {
    ...
    const timer = setTimeout(() => workspaceContext.run(workspace, () => {
      ...
      try {
        if (!state.active || !(virtualSoloAutomationEnabled() || state.roomSettings?.adminSimulation) || !state.roomSettings?.simulatedChat || state.chat?.enabled === false || state.chat?.locked) return;
...
function scheduleDemoResume() {
  if (!virtualSoloAutomationEnabled() || state.status !== 'paused' || state.game?.phase !== 'PAUSED') return;
  const workspace = currentWorkspace();
  const timer = setTimeout(() => workspaceContext.run(workspace, () => {
    forgetDemoTimer(workspace, timer);
    try {
      if ((workspace.isDemo || soloCommunityRoomEnabled()) && state.active && state.status === 'paused' && state.game && !prizeStatusPayload().bingo.closed) resumeRoom({ mode: 'automatic' });
    } catch (error) {
...
function autoResolveDemoClaimWindow(windowId) {
  if (!virtualSoloAutomationEnabled() || !windowId || state.claimWindow?.id !== windowId) return;
  try {
    while (true) {
      ...
      resolveClaim({ claimId: next.id, resolution: next.officialValid ? 'confirmed' : 'rejected', note: next.officialValid ? 'Validación automática de IA.' : 'Reclamo inválido.' });
    }
  } catch (error) {
...
function drawNextBall(source = 'manual') {
  ...
  if (!championshipRoomEnabled() && !flashRoomEnabled() && !antibingoRoomEnabled() && state.game.drawn.length >= state.game.mode && state.status === 'playing') {
    const graceMs = virtualSoloAutomationEnabled()
...
function updateDrawSettings(payload = {}) {
  if (!state.active || !state.game) throw new Error('No hay una sala abierta.');
  const minimumSeconds = currentWorkspace().isDemo ? 2 : 3;
  const nextSeconds = championshipRoomEnabled() ? CHAMPIONSHIP_AUTO_SECONDS : flashRoomEnabled() ? FLASH_AUTO_SECONDS : Math.max(minimumSeconds, Math.min(60, Number(payload.autoSeconds ?? state.game.autoSeconds) || 6));
...
const automaticPrizeAnnouncementTimers = new Map();

function clearAutomaticPrizeAnnouncementTimer(workspace = currentWorkspace()) {
  const timer = automaticPrizeAnnouncementTimers.get(workspace.id);
  if (timer) clearTimeout(timer);
  automaticPrizeAnnouncementTimers.delete(workspace.id);
}

function beginAutomaticPrizeAnnouncementShow(workspace = currentWorkspace()) {
  clearAutomaticPrizeAnnouncementTimer(workspace);
  const announcement = state.prizeAnnouncement;
  if (!state.active || !state.game || state.status !== 'paused' || state.pauseReason !== 'automatic_prize' || !announcement) return;
  const items = Array.isArray(announcement.items) ? announcement.items : [];
  if (!items.length) return finishAutomaticPrizeAnnouncement(workspace);
  announcement.currentIndex = Math.max(0, Math.min(items.length - 1, Number(announcement.currentIndex) || 0));
  announcement.stage = 'showing';
  announcement.showStartedAt = nowIso();
  announcement.showEndsAt = new Date(Date.now() + AUTO_PRIZE_SHOW_MS).toISOString();
  logEvent('automatic_prize_announcement_showing', { announcementId:announcement.id, itemId:items[announcement.currentIndex]?.id || '', type:items[announcement.currentIndex]?.type || '', winnerCount:(items[announcement.currentIndex]?.winners || []).length });
  saveState();
  broadcast();
  const timer = setTimeout(() => workspaceContext.run(workspace, () => advanceAutomaticPrizeAnnouncement(workspace)), AUTO_PRIZE_SHOW_MS);
  automaticPrizeAnnouncementTimers.set(workspace.id, timer);
}

function scheduleAutomaticPrizeAnnouncement(workspace = currentWorkspace()) {
  clearAutomaticPrizeAnnouncementTimer(workspace);
  const announcement = state.prizeAnnouncement;
  if (!state.active || !announcement || state.status !== 'paused' || state.pauseReason !== 'automatic_prize') return;
  if (announcement.stage === 'showing') {
    const due = new Date(announcement.showEndsAt || 0).getTime();
    const timer = setTimeout(() => workspaceContext.run(workspace, () => advanceAutomaticPrizeAnnouncement(workspace)), Math.max(0, (Number.isFinite(due) ? due : Date.now()) - Date.now()));
    automaticPrizeAnnouncementTimers.set(workspace.id, timer);
    return;
  }
  const due = new Date(announcement.announceAt || 0).getTime();
  const timer = setTimeout(() => workspaceContext.run(workspace, () => beginAutomaticPrizeAnnouncementShow(workspace)), Math.max(0, (Number.isFinite(due) ? due : Date.now()) - Date.now()));
  automaticPrizeAnnouncementTimers.set(workspace.id, timer);
}

function advanceAutomaticPrizeAnnouncement(workspace = currentWorkspace()) {
  clearAutomaticPrizeAnnouncementTimer(workspace);
  const announcement = state.prizeAnnouncement;
  if (!state.active || !announcement || state.status !== 'paused' || state.pauseReason !== 'automatic_prize') return;
  const items = Array.isArray(announcement.items) ? announcement.items : [];
  const nextIndex = (Number(announcement.currentIndex) || 0) + 1;
  if (nextIndex < items.length) {
    announcement.currentIndex = nextIndex;
    announcement.stage = 'lead_in';
    announcement.announceAt = new Date(Date.now() + AUTO_PRIZE_BETWEEN_MS).toISOString();
    announcement.showStartedAt = null;
    announcement.showEndsAt = null;
    saveState();
    broadcast();
    scheduleAutomaticPrizeAnnouncement(workspace);
    return;
  }
  finishAutomaticPrizeAnnouncement(workspace);
}

function finishAutomaticPrizeAnnouncement(workspace = currentWorkspace()) {
  clearAutomaticPrizeAnnouncementTimer(workspace);
  const announcement = state.prizeAnnouncement;
  if (!announcement) return;
  const hadBingo = Boolean(announcement.hasBingo || (announcement.items || []).some(item => item.type === 'bingo'));
  const resumeMode = announcement.resumeMode === 'manual' ? 'manual' : 'automatic';
  const announcementId = announcement.id;
  state.prizeAnnouncement = null;
  state.pauseReason = null;
  if (hadBingo) {
    const startedAt = nowIso();
    ...
    state.transition = {
      ...
      initialDrawnCount: state.game.drawn.length, remainingInitial,
      intervalMs: timing.intervalMs, leadInEndsAt: new Date(now + timing.leadInMs).toISOString(),
      endsAt: new Date(now + timing.totalMs).toISOString()
    };
    const bingoClaims = confirmedClaims('bingo').filter(claim => claim.tieGroupId === announcement.batchId || claim.claimWindowId === announcement.batchId);
    logEvent('bingo_confirmed_final_extraction', { claimIds:bingoClaims.map(claim => claim.id), tieCount:bingoClaims.length, remainingBalls:remainingInitial, leadInMs:timing.leadInMs, intervalMs:timing.intervalMs, automatic:true, afterAnnouncement:true });
    logEvent('automatic_prize_announcement_completed', { announcementId, bingo:true });
    saveState();
    broadcast();
    scheduleTransition();
    return;
  ...
  state.status = 'playing';
  state.game.drawMode = resumeMode;
  state.game.phase = resumeMode === 'automatic' ? 'DRAWING' : 'READY';
  logEvent('automatic_prize_announcement_completed', { announcementId, bingo:false, resumeMode });
  saveState();
  broadcast();
  if (state.game.drawn.length >= state.game.mode) {
    ...
    state.endedAt = nowIso();
    logEvent('game_finished', { round:state.round, balls:state.game.drawn.length, automatic:true, afterPrizeAnnouncement:true });
    archiveCurrentResults();
    saveState();
    broadcast();
    return;
  }
  if (resumeMode === 'automatic') scheduleAutomaticDraw();
}
...
function startRoom(payload = {}) {
  ...
  const baseStartSequenceMs = championshipRoomEnabled() ? CHAMPIONSHIP_START_SEQUENCE_MS : flashRoomEnabled() ? FLASH_START_SEQUENCE_MS : forcedSimulationStart ? (TEST_MODE ? 50 : 900) : currentWorkspace().isDemo ? (TEST_MODE ? 100 : DEMO_START_SEQUENCE_MS) : START_SEQUENCE_MS;
...
function resumeRoom(payload = {}) {
  ...
  if (state.pauseReason === 'automatic_prize' && state.prizeAnnouncement) throw new Error('Esperá a que termine el anuncio automático del premio.');
...
function archiveCurrentResults() {
  ...
  const meta = {
    ...
    roomType: state.roomSettings?.roomOrigin === 'community' ? (soloCommunityRoomEnabled() ? 'solo' : (state.roomSettings?.communityAccessType === 'private' ? 'private' : 'public')) : 'official',
...
function newRoomState() {
  ...
  clearAutomaticPrizeAnnouncementTimer();
...
function processAutomaticClaimsForCurrentBall() {
  ...
  clearWorkspaceTransitionTimer();
  clearAutomaticDrawTimer();
  clearClaimAutoResume();
  state.claimWindow = null;
  state.transition = null;
  const grouped = [];
  for (const claim of awarded) {
    const key = `${claim.type}:${Number(claim.prizeNumber) || 1}`;
    let item = grouped.find(entry => entry.key === key);
    if (!item) {
      item = { key, id:randomId('prize_item'), type:claim.type, prizeNumber:Number(claim.prizeNumber)||1, prizeLabel:claim.prizeLabel || prizeLabelFor(claim.type, claim.prizeNumber, state.game.mode), winners:[] };
      grouped.push(item);
    }
    item.winners.push({ claimId:claim.id, playerId:claim.playerId, playerName:claim.playerName, cardId:claim.cardId, cardNumber:claim.cardNumber });
  }
  const now = Date.now();
  state.status = 'paused';
  state.game.phase = 'PAUSED';
  state.pauseReason = 'automatic_prize';
  state.prizeAnnouncement = {
    id: randomId('prize_announcement'), batchId, ball: state.game.drawn.at(-1) ?? null, drawnCount: state.game.drawn.length,
    stage:'lead_in', currentIndex:0, createdAt:resolvedAt, announceAt:new Date(now + AUTO_PRIZE_LEAD_IN_MS).toISOString(),
    showStartedAt:null, showEndsAt:null, resumeMode:state.game.drawMode === 'manual' ? 'manual' : 'automatic',
    items: grouped.map(({key,...item}) => item), hasBingo: awarded.some(claim => claim.type === 'bingo')
  };
  logEvent('automatic_claim_batch_confirmed', {
    batchId, ball: state.game.drawn.at(-1) ?? null, drawnCount: state.game.drawn.length,
    announcementId:state.prizeAnnouncement.id,
  ...
  logEvent('automatic_prize_announcement_scheduled', { announcementId:state.prizeAnnouncement.id, items:grouped.length, leadInMs:AUTO_PRIZE_LEAD_IN_MS, showMs:AUTO_PRIZE_SHOW_MS });
  return awarded;
}
...
function createClaim(player, payload) {
  ...
  if (!existingWindow || !windowOpen) {
    state.claimWindow = {
      ...
      openedAt: nowIso(),
      openedAtMs: nowMs,
      expiresAtMs: nowMs + (virtualSoloAutomationEnabled() ? (TEST_MODE ? CLAIM_QUEUE_WINDOW_MS : DEMO_CLAIM_WINDOW_MS) : CLAIM_QUEUE_WINDOW_MS),
...
function championshipActaCsv() {
  ...
  const lines = [
    ...
    ['Puntuación', report.mode===75 ? 'Número +1 · Primera Línea +10 · 4 Esquinas +15 · Segunda Línea +20 · Triple Línea +30 · Cuádruple Línea +40 · Quinta Línea +50 · Bingo +60 · Bonus al primero: Línea +5 · 4 Esquinas +5 · Segunda Línea +5 · Triple Línea +5 · Cuádruple Línea +5 · Quinta Línea +5 · Bingo +15' : 'Número +1 · Primera Línea +10 · Segunda Línea +20 · Bingo +60 · Bonus al primero: Línea +5 · Segunda Línea +5 · Bingo +15'],
    ...
    [],
    ['CLASIFICACIÓN FINAL'],
    ['PUESTO','JUGADOR','POSICIÓN','ELEGIBLE','PUNTOS','BINGOS','BONUS 1ER BINGO','QUINTAS LÍNEAS','CUÁDRUPLES LÍNEAS','TRIPLES LÍNEAS','4 ESQUINAS','SEGUNDAS LÍNEAS','PRIMERAS LÍNEAS','NÚMEROS'],
    ...report.leaderboard.map(item => [item.rank,item.playerName,item.label,item.eligible?'SÍ':'NO',item.points,item.bingos,item.firstBingoBonusPoints ?? 0,item.quintupleLines||0,item.quadrupleLines||0,item.tripleLines||0,item.corners||0,item.secondLines,item.lines,item.hits]),
    [],
    ['PUNTOS POR RONDA'],
  ...
  for (const round of report.rounds || []) {
    ...
    lines.push(['RESULTADO DE LA RONDA'],['PUESTO','JUGADOR','POSICIÓN','PUNTOS','BINGO','BONUS 1ER BINGO','5ª LÍNEA','4ª LÍNEA','TRIPLE LÍNEA','2ª LÍNEA','4 ESQUINAS','1ª LÍNEA','NÚMEROS']);
    for (const item of round.roundLeaderboard || []) lines.push([item.rank,item.playerName,item.label,item.roundPoints,item.bingoBall||'',item.firstBingoBonus||0,item.quintupleLineBall||'',item.quadrupleLineBall||'',item.tripleLineBall||'',item.secondLineBall||'',item.cornersBall||'',item.lineBall||'',item.hitCount]);
  ...
  lines.push(['JUGADOR','POSICIÓN','RONDA','PUNTOS','NÚMEROS','LÍNEA BOLILLA','4 ESQUINAS BOLILLA','2ª LÍNEA BOLILLA','TRIPLE LÍNEA BOLILLA','4ª LÍNEA BOLILLA','5ª LÍNEA BOLILLA','BINGO BOLILLA','BONUS 1ER BINGO','MATRIZ']);
  for (const position of report.positions) {
    for (const round of position.rounds || []) {
      const matrix=(round.grid||[]).map(row=>(row||[]).map(v=>v==null?'-':v).join(' ')).join(' / ');
      lines.push([position.playerName,position.label,round.round,round.points,round.hitCount,round.lineBall||'',round.cornersBall||'',round.secondLineBall||'',round.tripleLineBall||'',round.quadrupleLineBall||'',round.quintupleLineBall||'',round.bingoBall||'',round.firstBingoBonus||0,matrix]);
    }
  }
  const quote = value => `"${String(value ?? '').replace(/"/g, '""')}"`;
  return '\ufeff' + lines.map(row => row.map(quote).join(';')).join('\r\n');
...
function ensureCommunityRoomInviteToken(record) {
  if (!record) return '';
  const current = String(record.inviteToken || '').trim();
  if (/^invite_[a-f0-9]{24}$/i.test(current)) return current;
  record.inviteToken = randomId('invite');
  record.updatedAt = nowIso();
  savePlatform();
  return record.inviteToken;
}

function communityRoomInviteTokenMatches(record, value) {
  if (!record || record.accessType !== 'private') return false;
  const expected = String(record.inviteToken || '').trim();
  const supplied = String(value || '').trim();
  if (!/^invite_[a-f0-9]{24}$/i.test(expected) || !/^invite_[a-f0-9]{24}$/i.test(supplied)) return false;
  return safeEqual(expected, supplied);
}

function communityCreatorShareUrl(record) {
  if (!record) return '';
  if (record.accessType !== 'private') return communityPublicShareUrl(record.id);
  const token = ensureCommunityRoomInviteToken(record);
  return `${communityPublicShareUrl(record.id)}?acceso=${encodeURIComponent(token)}`;
}
...
function createCommunityPublicRoom(payload = {}) {
  ...
  const creatorShareUrl = communityCreatorShareUrl(record);
  return {
    ...publicPayload,
    shareUrl:creatorShareUrl,
    enterUrl:creatorShareUrl,
    creatorCode,
...
function createCommunitySoloRoom(payload = {}) {
  const hostName = normalizeCommunityName(payload.name);
  if (SOLO_AI_NAMES.some(aiName => aiName.localeCompare(hostName, 'es', { sensitivity:'base' }) === 0)) {
    throw new Error('Ese nombre está reservado para uno de tus rivales del Modo Solitario.');
  }
  const mode = Number(payload.mode) === 75 ? 75 : 90;
  const gameKind = ['championship','flash','antibingo'].includes(payload.gameKind) ? payload.gameKind : 'normal';
  const maxCardsPerPlayer = gameKind === 'flash' ? 1 : Math.max(1, Math.min(MAX_CARDS_PER_PLAYER, Math.round(Number(payload.maxCardsPerPlayer) || 2)));
  const autoSeconds = gameKind === 'championship' ? CHAMPIONSHIP_AUTO_SECONDS : gameKind === 'flash' ? FLASH_AUTO_SECONDS : Math.max(4, Math.min(20, Math.round(Number(payload.autoSeconds) || 8)));
  let championshipRounds = 0;
  if (gameKind === 'championship') {
    const requestedRounds = Math.round(Number(payload.championshipRounds) || CHAMPIONSHIP_DEFAULT_ROUNDS);
  ...
  const rules = gameKind === 'championship'
    ...
    : ['flash','antibingo'].includes(gameKind)
      ...
      : mode === 90
        ? roomRulesFor(90, { ambocabeza:Boolean(requestedRules.ambocabeza), line:requestedRules.line !== false, bingo:true })
        : roomRulesFor(75, { line:requestedRules.line !== false, corners:Boolean(requestedRules.corners), doubleLine:Boolean(requestedRules.doubleLine), tripleLine:Boolean(requestedRules.tripleLine), bingo:true });
  const claimMode = ['championship','flash','antibingo'].includes(gameKind) ? 'manual' : (payload.claimMode === 'automatic_ties' ? 'automatic_ties' : 'manual');
  const cardCount = normalizedGeneratedCount(Math.max(25, Math.min(gameKind === 'championship' ? 250 : 100, 4 * maxCardsPerPlayer + 20)));
  const workspace = ensureWorkspace(`solo_${randomId('room')}`, null, `Solitario · ${hostName}`);
  let playerSessionToken = '';
  let humanPlayerId = '';
  workspaceContext.run(workspace, () => {
    createSimpleRoom({
      ...
      maxCardsPerPlayer,
      roomOrigin:'community',
      communityPlayMode:'solo',
      hostName,
      roomName:`Solitario de ${hostName}`,
      communityGameKind:gameKind,
      championshipRounds,
      communityStartMode:'manual',
      communityAccessType:'private',
      maxOpenPlayers:4
    });
    state.roomSettings.gameType = 'solo';
    state.roomSettings.joinOpen = true;
    state.roomSettings.simulatedChat = true;

    const human = openJoinPlayer({
      name:hostName,
      cardCount:maxCardsPerPlayer,
      deviceId:String(payload.visitorId || randomId('device')).slice(0,120),
      communityAccessGranted:true
    });
    playerSessionToken = human.token;
    const humanPlayer = state.players.find(item => item.sessionToken === playerSessionToken);
    if (!humanPlayer) throw new Error('No se pudo crear tu jugador solitario.');
    humanPlayerId = humanPlayer.id;
    state.communityCreatorPlayerId = humanPlayer.id;

    for (const aiName of SOLO_AI_NAMES) {
      state.roomSettings.joinOpen = true;
      const joined = openJoinPlayer({
        name:aiName,
        cardCount:maxCardsPerPlayer,
        deviceId:`solo-ai-${aiName.toLowerCase()}`,
        communityAccessGranted:true
      });
      const ai = state.players.find(item => item.sessionToken === joined.token);
      if (!ai) throw new Error(`No se pudo crear a ${aiName}.`);
      ai.virtual = true;
      ai.sessionToken = null;
      ai.sessionDeviceId = '';
      ai.openJoinDeviceId = '';
      ai.autoMark = true;
      ai.markingModeChosen = true;
      ai.requestedCardCount = maxCardsPerPlayer;
      ai.allowedCardCount = maxCardsPerPlayer;
      if (!championshipRoomEnabled()) {
        releaseReservationsForPlayer(ai);
        const chosen = chooseDiverseCardsForPlayer(maxCardsPerPlayer, mode);
        ai.cardIds = chosen.map(card => card.id);
        ai.selectedCardIds = [...ai.cardIds];
        ai.reservedCardIds = [];
        ai.offeredCardIds = [];
        ai.selectionConfirmed = true;
        ai.marks = Object.fromEntries(ai.cardIds.map(cardId => [cardId, []]));
        syncAutoMarksForPlayer(ai);
      }
    }
    state.roomSettings.joinOpen = false;
    updateCardDisplayNames();
    enforceAutoMarkPolicy();
    logEvent('community_solo_created', { hostName, gameKind, mode, championshipRounds, maxCardsPerPlayer, rivals:[...SOLO_AI_NAMES] });
    saveState(); broadcast();
  });
  return {
    ok:true,
    enterUrl:'/jugar',
    workspaceId:TEST_MODE ? workspace.id : undefined,
    playerId:TEST_MODE ? humanPlayerId : undefined,
    playerSessionToken,
    gameKind,
    mode,
    rivals:[...SOLO_AI_NAMES]
  };
}
...
function championshipOrganizerFromHistorical(record) {
  ...
  return {
    ...
    enterUrl:communityCreatorShareUrl(record), shareUrl:communityCreatorShareUrl(record), transmissionUrl:'',
    championship:{
...
function recoverCommunityCreator(payload = {}) {
  ...
  verifyCommunityCreatorCode(record, payload.creatorCode);
  const workspace = (record.workspaceId && workspaces.get(record.workspaceId)) || (record.roomCode ? findWorkspaceByRoomCode(record.roomCode) : null);
  // La vista del organizador consulta creator-state cada 5 s mientras está abierta.
  // Una consulta autenticada demuestra actividad real del creador y mantiene viva
  // una sala manual vacía. Si cierra esa vista, deja de renovar esta marca.
  if (workspace?.state?.active) workspace.lastActivityAt = Date.now();
  if (!workspace?.state?.active) {
    if (record.startMode === 'scheduled' && record.startsAt && record.status === 'scheduled') {
      return {
        ...
        shareUrl:communityCreatorShareUrl(record), transmissionUrl:''
      };
    }
...
function accessCommunityRoom(req, res, payload = {}) {
  ...
  if (record.accessType === 'private' && !reqMayAccessPrivateCommunityRoom(req, record)) verifyCommunityRoomAccessKey(record, payload.accessKey);
  const workspace = (record.workspaceId && workspaces.get(record.workspaceId)) || (record.roomCode ? findWorkspaceByRoomCode(record.roomCode) : null);
...
function prepareCommunityRematchWindow() {
  ...
  if (soloCommunityRoomEnabled()) { state.communityRematch = { disabled:true, reason:'solo', expiresAt:new Date(Date.now() + COMMUNITY_FINISH_GRACE_MS).toISOString(), createdAt:nowIso() }; return state.communityRematch; }
...
function maybeCloseInactiveEmptyCommunityRoom(workspace = currentWorkspace()) {
  if (!state.active || state.status !== 'waiting') return false;
  if (state.roomSettings?.roomOrigin !== 'community' || state.roomSettings?.communityStartMode !== 'manual') return false;
  if (registrationSummaryPayload().registeredPlayers > 0) return false;
  const lastActivityAt = Number(workspace?.lastActivityAt || 0);
  if (!lastActivityAt || Date.now() - lastActivityAt < COMMUNITY_EMPTY_MANUAL_IDLE_MS) return false;
  const publicId = String(state.roomSettings?.communityPublicId || '');
  state.closedReason = 'community_empty_inactive_timeout';
  state.roomSettings.joinOpen = false;
  logEvent('community_empty_room_inactive_timeout', { publicRoomId:publicId, idleMs:Date.now() - lastActivityAt, thresholdMs:COMMUNITY_EMPTY_MANUAL_IDLE_MS });
  closeRoom();
  return true;
}
...
function communityStatePayload(visitorId = '', req = null) {
  const id = String(visitorId || '').replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 80);
  if (id) communityVisitors.set(id, Date.now());
  pruneCommunityVisitors();
  const community = platform.community ||= blankCommunity();
  const roomGrant = req ? communityRoomGrantFromReq(req) : null;
  return {
    now: nowIso(),
    privateAccessPublicId: roomGrant?.publicId || '',
...
async function handleApi(req, res, url) {
  try {
    ...
    if (url.pathname === '/api/community/state' && req.method === 'GET') return sendJson(res, 200, communityStatePayload(url.searchParams.get('visitorId'), req));
    ...
    if (url.pathname === '/api/community/solo' && req.method === 'POST') {
      if (!consumeRate(req, 'community-solo-room', 30, 60 * 60 * 1000)) return sendJson(res, 429, { error: 'Se iniciaron demasiadas partidas solitarias desde esta conexión. Probá más tarde.' });
      const created = createCommunitySoloRoom(await readJson(req));
      setPlayerSessionCookie(req, res, created.playerSessionToken);
...
const server = http.createServer(async (req, res) => {
  ...
  if (publicRoomMatch && (req.method === 'GET' || req.method === 'HEAD')) {
    ...
    const suppliedInvite = String(url.searchParams.get('acceso') || '').trim();
    let privateInviteGranted = false;
    if (record.accessType === 'private' && suppliedInvite) {
      if (!communityRoomInviteTokenMatches(record, suppliedInvite)) {
        res.writeHead(303, { Location:`/comunidad?mesa=${encodeURIComponent(record.id)}&acceso=invalido`, 'Cache-Control':'no-store', 'Referrer-Policy':'no-referrer' });
        return res.end();
      }
      const grant = createCommunityRoomAccessGrant(record);
      setCommunityRoomAccessCookie(req, res, grant);
      privateInviteGranted = true;
    }

    const workspace = (record.workspaceId && workspaces.get(record.workspaceId)) || (record.roomCode ? findWorkspaceByRoomCode(record.roomCode) : null);
    const activeRoom = workspace?.state?.active && String(workspace.state.roomSettings?.communityPublicId || '') === String(record.id);
    if (activeRoom && record.roomCode && (record.accessType !== 'private' || privateInviteGranted || reqMayAccessPrivateCommunityRoom(req, record))) {
      res.writeHead(303, { Location:`/jugador?sala=${encodeURIComponent(record.roomCode)}&directo=1`, 'Cache-Control':'no-store', 'Referrer-Policy':'no-referrer' });
      return res.end();
    }
...
setInterval(() => {
  for (const workspace of workspaces.values()) {
    workspaceContext.run(workspace, () => {
      ...
      try { maybeCloseInactiveEmptyCommunityRoom(workspace); }
      catch (error) { console.error(`No se pudo limpiar la sala comunitaria vacía en ${workspace.id}:`, error.message); }
...
setInterval(() => {
  ...
  for (const workspace of [...workspaces.values()]) {
    const ephemeralIdleTtl = workspace.isDemo ? DEMO_IDLE_TTL_MS : (workspace.isSolo ? SOLO_IDLE_TTL_MS : 0);
    if ((workspace.isDemo || workspace.isSolo) && ((workspace.expiresAt && workspace.expiresAt <= now) || (workspace.lastActivityAt && ephemeralIdleTtl && now - workspace.lastActivityAt > ephemeralIdleTtl))) {
      clearAutomaticDrawTimer(workspace); clearWorkspaceTransitionTimer(workspace); clearAutomaticPrizeAnnouncementTimer(workspace); clearClaimAutoResume(workspace); clearDemoAutomationTimers(workspace); clearDemoStartTimer(workspace); workspaces.delete(workspace.id);
...
for (const workspace of workspaces.values()) workspaceContext.run(workspace, () => {
  ...
  if (state.active && state.game && state.status === 'paused' && state.pauseReason === 'automatic_prize' && state.prizeAnnouncement) scheduleAutomaticPrizeAnnouncement();
  else if (state.active && state.game && state.status === 'paused' && state.pauseReason === 'claim' && state.game.drawMode === 'automatic' && !confirmedClaims('bingo').length) scheduleClaimAutoResume();
