...
(async()=>{try{
  ...
  room=(await json('/api/admin/create-simple-room','POST',{mode:90,cardCount:100,autoSeconds:60,rules:{line:true,bingo:true},paymentMode:'paid',cardPrice:1000,paymentAlias:'lagorda.form',whatsapp:'5493757624388',markingMode:'normal',maxCardsPerPlayer:4,linePrizeCount:1},ah)).data;
  ...
  r=await fetch(base+'/jugador/salir',{headers:{Cookie:pc},redirect:'manual'});assert.equal(r.status,303);assert.equal(r.headers.get('location'),'/jugador');
