...
(async () => { try {
  ...
  out = await json('/api/admin/create-ai-simulation', { method: 'POST', headers: adminHeaders(admin), body: JSON.stringify({ playerCount: 60, mode: 90, autoSeconds: 6, presenter: 'vero', rules: { ambocabeza: true, line: true, bingo: true } }) }); assert.equal(out.response.status, 200, JSON.stringify(out.data)); state = out.data;
  assert.equal(state.roomSettings.adminSimulation, true); assert.equal(state.players.length, 60); assert(state.players.every(p => p.virtual === true)); assert(state.players.every(p => p.allowedCardCount >= 1 && p.allowedCardCount <= 4)); const activeCards = state.players.reduce((sum,p)=>sum+p.cardIds.length,0); assert(activeCards >= 60 && activeCards <= 240); assert.equal(state.preflight.activeCards, activeCards); assert(state.players.every((p,i)=>p.name===`IA ${String(i+1).padStart(2,'0')}`));
  ...
  out = await json('/api/admin/start', { method: 'POST', headers: adminHeaders(admin), body: '{}' }); assert.equal(out.response.status, 200, JSON.stringify(out.data)); await wait(140);
  for (const number of lineNumbers) { out = await json('/api/admin/draw', { method: 'POST', headers: adminHeaders(admin), body: JSON.stringify({ source: 'simulation-test' }) }); assert.equal(out.response.status, 200, JSON.stringify(out.data)); }
  await wait(100); state = (await json('/api/admin/state', { headers: adminHeaders(admin) })).data; assert(state.claims.some(claim => claim.status === 'pending' && claim.playerName.startsWith('IA ')), 'Las IA de simulación deben generar reclamos para que el administrador los resuelva.'); assert(!state.claims.some(claim => claim.status === 'confirmed'), 'La simulación administrativa no debe auto-confirmar premios.');
