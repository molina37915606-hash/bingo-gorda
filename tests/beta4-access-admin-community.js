...
(async()=>{try{
 ...
 const preview=await req('/api/admin/player-view-session',{method:'POST',headers:admin,body:JSON.stringify({playerId:add.data.player.id})});assert.equal(preview.response.status,200,JSON.stringify(preview.data));assert.equal(preview.data.readOnly,true);assert(preview.data.url.includes('session='));assert(!/codigo=|acceso=/i.test(preview.data.url));
 const previewState=await req('/api/player/state',{headers:{'X-Player-Token':preview.data.token}});assert.equal(previewState.response.status,200,JSON.stringify(previewState.data));assert.equal(previewState.data.adminPreview,true);
 const previewMutation=await req('/api/player/automark',{method:'POST',headers:{'Content-Type':'application/json','X-Player-Token':preview.data.token},body:JSON.stringify({enabled:true})});assert.equal(previewMutation.response.status,403);
 ...
 const view=await req('/api/admin/player-view-session',{method:'POST',headers:admin,body:JSON.stringify({playerId:ai.id})});assert.equal(view.response.status,200,JSON.stringify(view.data));assert.equal(view.data.readOnly,false);assert(view.data.url.includes('simcontrol=1'));assert(!/codigo=|acceso=/i.test(view.data.url));
 const aiState=await req('/api/player/state',{headers:{'X-Player-Token':view.data.token}});assert.equal(aiState.response.status,200,JSON.stringify(aiState.data));assert.equal(aiState.data.player.id,ai.id);
 const adminJs=fs.readFileSync(path.join(__dirname,'..','js','admin-simplificado.js'),'utf8');
 assert(adminJs.includes('COPIAR CÓDIGO'));assert(adminJs.includes('LINK DIRECTO'));assert(adminJs.includes('¡La partida ya está lista!'));const adminHtml=fs.readFileSync(path.join(__dirname,'..','admin.html'),'utf8');assert(adminHtml.includes('VISTA PREVIA MÓVIL'));
