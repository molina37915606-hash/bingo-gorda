...
async function finishRound(admin, playerToken){for(let i=0;i<120;i++){let state=await ok('/api/player/state',{playerToken});if(state.championship?.betweenRounds||state.status==='finished')return state;if(state.championship?.closingClaims){await wait(35);continue}if(state.championship?.inTiebreak){const t=await raw('/api/admin/draw',{method:'POST',token:admin,body:{source:'v4-tiebreak'}});if(!t.response.ok){await wait(20);continue}state=await ok('/api/player/state',{playerToken});if(state.status==='finished')return state;continue}const draw=await raw('/api/admin/draw',{method:'POST',token:admin,body:{source:'v4-final-acta'}});if(!draw.response.ok){await wait(20);continue}}return ok('/api/player/state',{playerToken});}
