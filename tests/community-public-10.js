...
(async()=>{try{
  ...
  const rooms=[],guestNames=['Pedro','Lucía','Carlos','Marta','Nora','Ana','Beto','Sofía','Diego','Elena'];
  for(let i=0;i<10;i++){
    const response=await raw('/api/community/public-room',{method:'POST',body:{visitorId:`creator-${i}`,name:`Creador ${i+1}`,roomName:`Mesa pública ${i+1}`,mode:i%2?75:90,maxPlayers:10,maxCardsPerPlayer:1,autoSeconds:20,linePrizeCount:1,rules:{line:true,bingo:true},startMode:'manual'}});
  ...
  // Cada chat queda dentro de su propia sala.
  ...
  // Las diez pueden iniciar a la vez; cada creador es quien da la orden.
  for(const room of rooms)await ok('/api/player/community-start',{method:'POST',cookie:room.creatorCookie,body:{}});
  await wait(350);
  ...
  let manager=await ok('/api/admin/workspaces',{adminToken:admin});assert.equal(manager.rooms.length,10);assert.equal(manager.rooms.filter(x=>x.active).length,10,'Los diez workspaces deben estar activos.');
  // Una extracción en una sala no cambia las otras.
  ...
  // Un reclamo en una sala no pausa ni aparece en otra.
  ...
  // Reinicio del proceso: las diez salas y sus chats deben recuperarse.
  ...
  console.log('PRUEBA 10 SALAS PÚBLICAS: OK · límite 10 + chats + bolillas + reclamos aislados + reinicio');
