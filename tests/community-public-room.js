...
(async()=>{try{
  ...
  const creatorForChoose=await ok('/api/player/state',{cookie:creatorCookie});assert.equal(creatorForChoose.roomSettings.maxCardsPerPlayer,4,'La sala creada debe admitir hasta 4 cartones por jugador.');const creatorOffers=creatorForChoose.player.offeredCards.slice(0,4).map(x=>x.id);assert.equal(creatorOffers.length,4);await ok('/api/player/choose',{method:'POST',cookie:creatorCookie,body:{cardIds:creatorOffers}});
  ...
  creatorState=await ok('/api/player/state',{cookie:creatorCookie});assert.equal(creatorState.communityRoom.canStart,true,'Cuando todos confirmaron cartones, el creador debe poder iniciar.');assert.equal(creatorState.communityRoom.pendingSelectionPlayers,0);assert.deepEqual(creatorState.communityRoom.creatorPlayers.map(x=>[x.name,x.cardCount,x.ready]),[['Marta',4,true],['Pedro',1,true]],'El creador debe ver nombre y cantidad exacta de cartones de cada jugador.');
