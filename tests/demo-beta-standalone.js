...
const jsPath = path.join(ROOT, 'js', 'demo-beta.js');
const source = fs.readFileSync(jsPath, 'utf8');

assert(html.includes('BINGO DE LA GORDA BETA'), 'La DEMO debe identificarse como BETA.');
assert(html.includes('/js/demo-beta.js'), 'La DEMO debe usar su script independiente.');
assert(!html.includes('/demo/start'), 'La DEMO BETA no debe crear salas del servidor.');
assert(!html.includes('online-room-player.js'), 'La DEMO BETA no debe cargar el cliente normal del jugador.');
assert(!/\bfetch\s*\(/.test(source), 'La DEMO BETA no debe depender de fetch para jugar.');
assert(!source.includes('/api/'), 'La DEMO BETA no debe depender de APIs de sala.');
...
vm.runInContext(exposed, context, { filename: 'demo-beta.js' });
...
console.log('OK demo-beta-standalone: DEMO independiente y motor local verificados.');
