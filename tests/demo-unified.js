...
(async()=>{try{await waitServer();
 ...
 const page=await fetch(base+loc,{headers:{Cookie:cookie}});const html=await page.text();assert(page.ok);assert(html.includes('/js/player.js?v=cuasifinal-1'));assert(html.includes('/css/platform.css?v=cuasifinal-1'));assert(!html.includes('demo-alfa.js'));
