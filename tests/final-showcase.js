...
(async()=>{try{
  ...
  out = await json('/api/admin/add-official-player',{method:'POST',headers:adminHeaders(admin),body:JSON.stringify({name:'Segundo Jugador',cardCount:1})}); assert.equal(out.response.status,200,JSON.stringify(out.data));
  ...
  tx=(await json(`/api/broadcast/state?token=${encodeURIComponent(txToken)}`)).data; assert.equal(tx.status,'finished'); assert.equal(tx.resultsReady,true); assert(Array.isArray(tx.finalShowcase)&&tx.finalShowcase.length>=1,'La transmisión debe recibir los cartones ganadores.');
