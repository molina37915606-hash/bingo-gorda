...
(async()=>{try{
 ...
 const {created,a,b}=await createAnti(75,'tie',1);await selectRoom(admin,created.roomCode);let sa=await ok('/api/player/state',{playerToken:a.token}),sb=await ok('/api/player/state',{playerToken:b.token});const an=nums(sa.player.cards[0]),bn=nums(sb.player.cards[0]),common=an.filter(n=>bn.includes(n));assert(common.length,'Los cartones 75 deben compartir al menos un número para esta prueba');const last=common[0],seq=[...new Set([...an.filter(n=>n!==last),...bn.filter(n=>n!==last)]),last];await ok('/api/admin/test/draw-order',{method:'POST',token:admin,body:{sequence:seq}});for(let i=0;i<seq.length;i++)await ok('/api/admin/draw',{method:'POST',token:admin,body:{source:'anti-tie'}});sa=await ok('/api/player/state',{playerToken:a.token});assert.equal(sa.status,'paused');assert.equal(sa.antibingo.stage,'resolving_tie');assert.equal(sa.antibingo.tie,false);assert.equal(sa.antibingo.tieOccurred,true);assert.equal(sa.antibingo.tieResolution?.stage,'resolving');assert.equal(sa.antibingo.tieResolution?.candidateCardIds?.length,2);for(let i=0;i<50&&sa.status!=='finished';i++){await wait(20);sa=await ok('/api/player/state',{playerToken:a.token})}assert.equal(sa.status,'finished');assert.equal(sa.antibingo.tie,false);assert.equal(sa.antibingo.tieOccurred,true);assert.equal(sa.antibingo.aliveCount,0);assert.equal(sa.antibingo.winners.length,1);assert.equal(sa.antibingo.tieResolution?.stage,'resolved');assert(sa.antibingo.tieResolution?.criterionLabel);assert(sa.antibingo.tieResolution?.reason);assert.equal(sa.antibingo.winningBall,last);
 ...
 console.log('HOTFIX 05 ANTIBINGO: OK · 75/90 · multi-cartón · eliminación automática · último sobreviviente · empate resuelto por resistencia · Sobrevivientes · animación · TV');
