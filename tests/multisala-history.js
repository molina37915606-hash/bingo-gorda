...
(async()=>{try{
  ...
  await select(token,'slot2');const waiting=await ok('/api/admin/create-simple-room',{method:'POST',token,body:{mode:90,cardCount:30,autoSeconds:8,rules:{line:true,bingo:true},maxCardsPerPlayer:2}});const cancelledCode=waiting.roomCode;
  ...
  let agenda=await ok('/api/admin/community/schedule',{method:'POST',token,body:{action:'save',startsAt:starts1,registrationMinutes:15,autoStart:false,mode:90,markingMode:'normal',maxCardsPerPlayer:2,cardCount:50,autoSeconds:8,linePrizeCount:1,rules:{line:true,bingo:true}}});
  ...
  agenda=await ok('/api/admin/community/schedule',{method:'POST',token,body:{action:'save',id:schedule.id,startsAt:starts2,registrationMinutes:25,autoStart:false,mode:90,markingMode:'normal',maxCardsPerPlayer:3,cardCount:100,autoSeconds:12,linePrizeCount:2,rules:{line:true,bingo:true}}});
  ...
  agenda=await ok('/api/admin/community/schedule',{method:'POST',token,body:{action:'save',startsAt:autoStarts1,registrationMinutes:1,autoStart:true,mode:90,markingMode:'normal',maxCardsPerPlayer:2,cardCount:50,autoSeconds:60,linePrizeCount:1,rules:{line:true,bingo:true}}});const autoOne=agenda.scheduledGames.find(x=>x.startsAt===autoStarts1);assert(autoOne);
  agenda=await ok('/api/admin/community/schedule',{method:'POST',token,body:{action:'save',startsAt:autoStarts2,registrationMinutes:1,autoStart:true,mode:75,markingMode:'normal',maxCardsPerPlayer:2,cardCount:50,autoSeconds:60,linePrizeCount:1,rules:{line:true,bingo:true}}});const autoTwo=agenda.scheduledGames.find(x=>x.startsAt===autoStarts2);assert(autoTwo);
