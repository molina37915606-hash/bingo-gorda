...
const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'bingo-v10-7-test-'));
...
(async () => {
  try {
    ...
    assert.strictEqual(result.data.version, '10.7');
    ...
    const game = {
      id: 'game-v10-7', number: 17, mode: 90,
    ...
    // Segunda línea para otro jugador y cierre del botón de línea.
    const p3Card = state.players[2].cardIds[0];
    result = await json('/api/player/claim', {
      method: 'POST', headers: sessions[2].headers,
      body: JSON.stringify({ cardId: p3Card, type: 'line' })
    });
    assert.strictEqual(result.response.status, 200, JSON.stringify(result.data));
    ...
    assert.strictEqual(acta.data.version, '10.7');
    ...
    assert.strictEqual(result.response.status, 200);
    assert.strictEqual(result.data.format, 'el-bingo-de-la-gorda-v10-7-backup');
