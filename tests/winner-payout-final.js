...
(async()=>{try{
  ...
  let admin=await post('/api/admin/create-simple-room',{mode:75,cardCount:60,autoSeconds:60,rules:{line:true,corners:false,doubleLine:false,tripleLine:false,bingo:true},paymentMode:'free',markingMode:'normal',maxCardsPerPlayer:1},ah);
  assert.equal(admin.roomSettings.whatsapp,'+54 9 3757 624388','Una partida gratuita debe heredar el WhatsApp de Comunidad para consultas y premios');
  admin=await post('/api/admin/join-open',{open:true},ah);
  const winnerCookie=await join(admin.roomCode,'María Ganadora');
  let winner=await get('/api/player/state',{Cookie:winnerCookie});
  ...
  let other=await get('/api/player/state',{Cookie:otherCookie});
